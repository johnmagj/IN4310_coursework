...
class ImageCaptionModel(nn.Module):
    ...
    def forward(self, cnn_features, token_ids, is_train):
        ...
        if self.use_attention:
            ...
            # DONE: Use self.avgpool to convert features from shape [batch_size, 512, 7, 7] to [batch_size, 512]
            cnn_features = self.avgpool(cnn_features)   # Now the shape is [batch_size, 512, 1, 1]
            cnn_features = torch.flatten(cnn_features, start_dim=1) # Now the shape is [batch_size, 512]
...
class CaptionRNN(nn.Module):
    def __init__(self, cnn_feature_dim, vocabulary_size, embedding_size,
        ...
        # DONE: The output layer (final layer) is a linear layer. What should be the size (dimensions) of its output?
    ...
    def forward(self, tokens, features, is_train):
        ...
        # DONE: Initialize hidden_states with correct dimensions depending on the cell type.
        ...
        # DONE: Fetch the first (index 0) embeddings that should go as input to the RNN.
        ...
        for t in range(seq_len):
            ...
            for layer in range(self.num_layers):
                if layer == 0:
                    ...
                    cell_input = new_states[layer-1][:, :self.hidden_state_size] # DONE: Initialise to the output of the previous layer

                new_state = self.cells[layer](cell_input, hidden_states[layer]) # DONE: Call the cell for this layer
...
class LSTMCell(nn.Module):
    def __init__(self, input_size: int, hidden_state_size: int):
        ...
        # DONE: Initialise weights and biases for the forget gate (weight_f, bias_f), input gate (w_i, b_i),
        ...
        _n = hidden_state_size + input_size
        _var = 1.0/_n
        _std = _var**0.5  # To skip dependencies
        def _weight_generator():
            return nn.Parameter(torch.normal(mean=0.0, std=_std, size=(hidden_state_size + input_size, hidden_state_size)))
        
        def _bias_generator():
            return nn.Parameter(torch.ones(1, hidden_state_size)*2.0)

        # Forget gate parameters
        self.weight_f = _weight_generator()
        self.bias_f = _bias_generator()
        
        # Input gate parameters
        self.weight_i = _weight_generator()
        self.bias_i = _bias_generator()
        
        # Output gate parameters
        self.weight_o = _weight_generator()
        self.bias_o = _bias_generator()
        
        # Memory cell parameters
        self.weight = _weight_generator()
        self.bias = _bias_generator()
